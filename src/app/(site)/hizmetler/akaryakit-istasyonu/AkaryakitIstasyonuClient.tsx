...
import {
  ChevronRight, Fuel, Leaf, Shield, Zap, Star, Phone,
  CheckCircle, ArrowRight, X, ZoomIn, ChevronLeft,
  Gauge, Wrench, Clock, Award, Droplets,
  Car, BusFront, MapPin, Crown, Heart, Cpu, TrendingUp,
  GraduationCap, Bus, MapPinned, Briefcase, Mountain,
  Anchor, Camera, Coffee, Flag, Gem, Map, Medal, Music,
  Palmtree, Rocket, Sun, Trophy, Eye, Target, Lightbulb,
  Wallet, Compass, Users, Sparkles, HeartHandshake, Key,
  Globe, Hotel, FileText, Plane, Ship, Mic2,
} from 'lucide-react'
...
const ICON_MAP: Record<string, LucideIcon> = {
  ...
  Car, Ship, Mic2, ArrowRight, Key, Wrench, HeartHandshake, Globe, Hotel, FileText,
  Fuel, Gauge, Award, Droplets,
}
...
export default function AkaryakitIstasyonuClient({ initialContent }: { initialContent: BilingualContent }) {
      ...
      <div className="relative bg-gray-900 pt-28 sm:pt-36 pb-20 overflow-hidden" style={{ minHeight: 480 }}>
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10">
          <div className="flex items-center gap-1.5 text-sm mb-8">
            ...
            <ChevronRight className="w-3.5 h-3.5 text-gray-600" />
            <span className="text-amber-400 font-semibold">{lang === 'tr' ? 'Akaryakıt İstasyonu' : 'Fuel Station'}</span>
          </div>
          ...
          <motion.div {...fadeUp()}>
            <div className="inline-flex items-center gap-2 bg-amber-500/15 border border-amber-500/30 rounded-full px-4 py-1.5 mb-5">
              ...
              <span className="text-amber-400 text-xs font-bold tracking-widest">{v('hero', 'badge', 'Hizmetler')}</span>
            </div>
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-white mb-2 leading-tight">
              {v('hero', 'title', '')}
            </h1>
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-black text-amber-400 mb-4 leading-tight">
              {v('hero', 'title2', '')}
            </h2>
          ...
          <motion.div {...fadeUp()} className="grid lg:grid-cols-[1fr_340px] gap-12 items-start">
            ...
            <div className="space-y-5">
              <div className="bg-gradient-to-br from-amber-500 to-amber-600 rounded-3xl p-6 text-center shadow-lg shadow-amber-500/20">
                ...
                <h3 className="font-black text-white text-base mb-1">{v('cta', 'cta_title', lang === 'tr' ? 'Kurumsal Anlaşma' : 'Corporate Agreement')}</h3>
              ...
              {advantages.length > 0 && (
                <div className="bg-gray-50 border border-gray-100 rounded-3xl p-7">
                  ...
                  <h3 className="font-black text-gray-900 text-base mb-4">{v('advantages', 'adv_title', '')}</h3>
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.92, y: 24 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.92, y: 24 }} transition={{ duration: 0.28 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
              <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 pointer-events-auto relative max-h-[90vh] overflow-y-auto">
                ...
                <p className="text-sm text-gray-500 mb-6">{lang === 'tr' ? 'Kurumsal yakıt anlaşması için özel fiyat teklifi alın.' : 'Get a custom price quote for your corporate fuel agreement.'}</p>
                <QuoteForm source="akaryakit-istasyonu" onSuccess={() => setTimeout(() => setShowModal(false), 2500)} />
              </div>
            </motion.div>
          </>
        )}
