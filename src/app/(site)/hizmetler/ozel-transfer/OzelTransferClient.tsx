...
import {
  ChevronRight, Car, BusFront, MapPin, Shield, Star, Phone,
  CheckCircle, ArrowRight, X, ZoomIn, ChevronLeft, Clock,
  Sparkles, Key, Users, Wrench, HeartHandshake, Crown,
  Heart, Cpu, TrendingUp, GraduationCap, Bus, MapPinned,
  Briefcase, Zap, Mountain, Anchor, Camera, Coffee, Flag, Gem,
  Leaf, Map, Medal, Music, Palmtree, Rocket, Sun, Trophy,
  Eye, Target, Lightbulb, Wallet, Compass, Plane, Ship, Mic2,
} from 'lucide-react'
...
const ICON_MAP: Record<string, LucideIcon> = {
  ...
  Car, Ship, Mic2, ArrowRight, Key, Wrench, HeartHandshake,
}
...
export default function OzelTransferClient({ initialContent }: { initialContent: BilingualContent }) {
      ...
      <div className="relative bg-gray-900 pt-36 pb-20 overflow-hidden" style={{ minHeight: 480 }}>
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10">
          <div className="flex items-center gap-1.5 text-sm mb-8">
            ...
            <ChevronRight className="w-3.5 h-3.5 text-gray-600" />
            <span className="text-amber-400 font-semibold">{lang === 'tr' ? 'Özel Transfer Hizmetleri' : 'Private Transfer Services'}</span>
          </div>
          ...
          <motion.div {...fadeUp()}>
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-white mb-2 leading-tight">
              {v('hero', 'title', lang === 'tr' ? 'Yolculuğunuza' : 'Add Value')}
            </h1>
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-black text-amber-400 mb-4 leading-tight">
              {v('hero', 'title2', lang === 'tr' ? 'Değer Katın!' : 'To Your Journey!')}
            </h2>
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.92, y: 24 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.92, y: 24 }} transition={{ duration: 0.28 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
              <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 pointer-events-auto relative max-h-[90vh] overflow-y-auto">
                ...
                <p className="text-sm text-gray-500 mb-6">{lang === 'tr' ? 'VIP transfer ve özel hizmetler için size özel fiyat teklifi.' : 'Custom quote for VIP transfer and private services.'}</p>
                <QuoteForm source="ozel-transfer" onSuccess={() => setTimeout(() => setShowModal(false), 2500)} />
              </div>
            </motion.div>
          </>
        )}
