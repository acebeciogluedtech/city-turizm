...
import {
  ...
  Leaf, Map, Medal, Music, Palmtree, Rocket, Sun, Trophy,
} from 'lucide-react'
...
export default function PersonelTasimacilikClient({ initialContent }: { initialContent: BilingualContent }) {
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <div className="relative bg-gray-900 pt-28 sm:pt-36 pb-20 overflow-hidden" style={{ minHeight: 480 }}>
        {v('hero', 'hero_img') && (
          <img src={v('hero', 'hero_img')} alt="Personel Taşımacılığı" className="absolute inset-0 w-full h-full object-cover object-center animate-fade-in" />
        )}
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10">
          <div className="flex items-center gap-1.5 text-sm mb-8">
            ...
            <ChevronRight className="w-3.5 h-3.5 text-gray-600" />
            <span className="text-amber-400 font-semibold">{v('hero', 'title', lang === 'tr' ? 'Personel Taşımacılığı' : 'Personnel Transportation')}</span>
          </div>
          ...
          <motion.div {...fadeUp()}>
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-white mb-4 leading-tight">
              {v('hero', 'title', 'Personel Taşımacılığı')}
            </h1>
            ...
            <div className="space-y-5">
              ...
              <div className="bg-gradient-to-br from-amber-500 to-amber-600 rounded-3xl p-6 text-center shadow-lg shadow-amber-500/20">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 bg-white text-amber-600 font-bold text-sm py-3 rounded-xl
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.22 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
              <div className="relative w-full max-w-4xl pointer-events-auto">
                <button onClick={() => setLightbox(null)} className="absolute -top-12 right-0 w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors">
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.92, y: 24 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.92, y: 24 }} transition={{ duration: 0.28 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
              <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 pointer-events-auto relative">
                <button onClick={() => setShowModal(false)} className="absolute top-5 right-5 w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors">
                ...
                <p className="text-sm text-gray-500 mb-6">{lang === 'tr' ? 'Personel taşımacılığı için kurumunuza özel fiyat teklifi.' : 'Custom quote for personnel transportation.'}</p>
                <QuoteForm source="personel-tasimacilik" onSuccess={() => setTimeout(() => setShowModal(false), 2500)} />
              </div>
            </motion.div>
          </>
        )}
