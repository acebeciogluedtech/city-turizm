          ...
          {isMobileOpen && (
            ...
            >
              <div className="container mx-auto px-4 py-3 space-y-0.5">
                ...
                {/* Telefon + Sosyal Medya - en altta */}
                <div className="mx-2 mb-3 mt-1 rounded-2xl bg-gray-50 border border-gray-100 px-4 py-4 space-y-3">
                  {/* Telefon numaraları */}
                  <div className="space-y-1.5">
                    <a href={cm('topbar','phone1_href','tel:4441289')}
                       className="flex items-center gap-2.5 text-amber-600 font-black text-base">
                      <Phone className="w-4 h-4 flex-shrink-0" />
                      {cm('topbar','phone1','444 1 289')}
                    </a>
                    {cm('topbar','phone2','') && (
                      <a href={cm('topbar','phone2_href','tel:+902125438097')}
                         className="flex items-center gap-2 text-gray-500 font-medium text-sm">
                        <Phone className="w-3.5 h-3.5 flex-shrink-0 text-gray-400" />
                        {cm('topbar','phone2','+90 212 543 80 97')}
                      </a>
                    )}
                    {cm('topbar','phone3','') && (
                      <a href={cm('topbar','phone3_href','tel:+902125836161')}
                         className="flex items-center gap-2 text-gray-500 font-medium text-sm">
                        <Phone className="w-3.5 h-3.5 flex-shrink-0 text-gray-400" />
                        {cm('topbar','phone3','+90 212 583 61 61')}
                      </a>
                    )}
                  </div>
                  {/* Sosyal medya */}
                  <div className="flex items-center gap-2 pt-1">
                    {[
                      ...
                      <a key={s.title} href={cm('social', s.key, `https://${s.key}.com`)}
                         target="_blank" rel="noopener noreferrer" title={s.title}
                         onClick={() => setIsMobileOpen(false)}
                         className="w-9 h-9 rounded-xl bg-white border border-gray-200 hover:border-amber-300 flex items-center justify-center
                                    text-gray-500 hover:text-amber-600 transition-colors shadow-sm">
