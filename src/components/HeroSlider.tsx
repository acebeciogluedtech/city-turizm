...
import { motion, useTransform, AnimatePresence, useInView, useMotionValue, useMotionValueEvent, animate } from 'framer-motion'
...
function HeroStats() {
  ...
  return (
    <div ref={ref} className="grid grid-cols-2 2xl:grid-cols-4 gap-2 mb-4 xl:mb-6">
      {STATS.map((s, i) => (
        <motion.div key={i}
          initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ delay: i * 0.1, duration: 0.4 }}
          className="group flex items-center gap-2 px-3 py-2.5 rounded-xl bg-gray-50 hover:bg-amber-50 border border-gray-100 hover:border-amber-200 transition-all duration-300"
        >
          <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 bg-amber-100/80">
            <s.icon className="w-4 h-4 text-amber-600" strokeWidth={2} />
          </div>
          <div className="min-w-0">
            <div className="text-base font-black leading-none text-gray-900">{s.value}</div>
            <div className="text-[10px] mt-0.5 font-medium leading-tight text-gray-400 group-hover:text-amber-600 transition-colors">{s.label}</div>
          </div>
        </motion.div>
      ))}
    </div>
...
function RotatingTag() {
  ...
  return (
    <div className="flex items-center gap-3 mb-3 xl:mb-6 h-10 overflow-hidden">
...
function VideoFrame({
  ...
  return (
    ...
    >
      ...
      {/* Pointer-block overlay — stops any YouTube hover/click/touch UI */}
      <div
        className="absolute inset-0 z-[30]"
        style={{ cursor: 'default', touchAction: 'none', WebkitTapHighlightColor: 'transparent', background: 'rgba(0,0,0,0)' }}
      />
...
function ScrollIndicator({ progress, scrollHint }: { progress: MotionValue<number>; scrollHint: string }) {
  ...
  return (
    <div
      ...
      className="absolute bottom-8 left-[71%] -translate-x-1/2 z-[25] flex flex-col items-center gap-2 pointer-events-none"
    >
      ...
      <span className="text-gray-500 text-xs font-semibold tracking-[0.15em]">{scrollHint}</span>
    </div>
  )
}
...
function DesktopHero({ onApply }: { onApply: () => void }) {
  ...
  const animCtrlRef = useRef<ReturnType<typeof animate> | null>(null)
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      animCtrlRef.current = animate(progress, newP, {
  ...
  const CARD_BOTTOM = '32px'
  const CARD_LEFT   = '44%'
  const CARD_RIGHT  = '2%'

  const cardLeft   = useTransform(progress, [0, 1], [CARD_LEFT,   '0%'])
  const cardTop    = useTransform(progress, [0, 1], [CARD_TOP,    `${NAV_HEIGHT}px`])
  ...
  return (
    <div className="relative h-screen bg-white">
      ...
      >
        <RotatingTag />
        <h1 className="text-[2rem] xl:text-5xl 2xl:text-6xl font-black text-gray-900 leading-[1.05] mb-3 xl:mb-5">
          {titleParts.map((p, i) => (
            <span key={i}>{p}{i < titleParts.length - 1 && <br />}</span>
          ))}
        </h1>
        <p className="text-gray-500 text-sm xl:text-base leading-relaxed mb-4 xl:mb-8 max-w-sm">{desc}</p>
        <HeroStats />
