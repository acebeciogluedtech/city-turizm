...
import { ChevronRight, Bus, Briefcase, Shield, Heart, Globe, Compass, Crown, Clock, ShieldCheck, Sparkles, Wallet, MapPinned, Star, Plane, Mountain, Anchor, Camera, Coffee, Flag, Gem, Leaf, Map, Medal, Music, Palmtree, Rocket, Sun, Trophy, Users, Zap } from 'lucide-react'
...
const ICON_MAP: Record<string, LucideIcon> = {
  ...
  Flag, Gem, Leaf, Map, Medal, Music, Palmtree, Rocket, Sun, Trophy, Users, Zap,
}
const DEFAULT_TAGS = [
    ...
    <div ref={ref} className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3 mb-8">
      {STATS.map((s, i) => (
        <motion.div key={i}
          ...
          className={`group flex items-center gap-2 md:gap-3 px-3 md:px-4 py-2.5 md:py-3 rounded-xl transition-all duration-300
            ${dark ? 'bg-white/5 border border-white/10 hover:bg-white/10' : 'bg-gray-50 hover:bg-amber-50 border border-gray-100 hover:border-amber-200'}`}
        >
          <div className={`w-8 h-8 md:w-10 md:h-10 rounded-lg flex items-center justify-center flex-shrink-0
            ${dark ? 'bg-amber-400/15' : 'bg-amber-100/80'}`}>
            <s.icon className={`w-4 h-4 md:w-5 md:h-5 ${dark ? 'text-amber-400' : 'text-amber-600'}`} strokeWidth={2} />
          </div>
          <div className="min-w-0">
            <div className={`text-base md:text-xl font-black leading-none ${dark ? 'text-white' : 'text-gray-900'}`}>{s.value}</div>
            <div className={`text-[10px] md:text-xs mt-0.5 font-medium truncate
              ${dark ? 'text-white/50' : 'text-gray-400 group-hover:text-amber-600'}`}>{s.label}</div>
          </div>
        </motion.div>
      ))}
    </div>
...
// ── VideoFrame ────────────────────────────────────────────────────────────────
// Orange/logo loading placeholder until video is ACTUALLY playing (state=1).
// visibility:hidden on iframe wrapper prevents ANY YouTube UI from appearing.
function VideoFrame({ onPlayerReady, logoUrl }: { onPlayerReady?: (p: any) => void; logoUrl?: string } = {}) {
    ...
    const create = () => {
      ...
      window._ytPlayers[pid] = new window.YT.Player(pid, {
        ...
        events: {
          ...
          onStateChange: (e: any) => {
            // Only reveal video when ACTUALLY playing — hides all YouTube native UI
  ...
  return (
    <div className="absolute inset-0 overflow-hidden" style={{ background: 'linear-gradient(135deg,#f59e0b 0%,#ea580c 100%)' }}>

      {/* iframe — hidden until video is playing */}
      <div style={{
        position: 'absolute', top: '-8px', right: '-8px', bottom: '-8px', left: '-8px',
        overflow: 'hidden', pointerEvents: 'none',
        visibility: isReady ? 'visible' : 'hidden',
      }}>
        <div id={pid} className="absolute border-0" style={{
          top: '50%', left: '50%',
      ...
      {/* Pointer-block overlay — prevents YouTube hover UI */}
      <div className="absolute inset-0 z-10" style={{ cursor: 'default' }} />

      {/* Orange logo placeholder — fades out when video starts */}
      <div
  ...
  return (
    <section className="lg:hidden bg-white pt-[72px]">
      <div className="relative mx-4 mt-4 rounded-2xl overflow-hidden shadow-xl" style={{ height: '112vw', minHeight: 440 }}>
        ...
        {(!videoReady || !isPlaying) && <div className="absolute inset-0 z-25 bg-black/60" />}
        <button
          onClick={() => {
...
function ScrollIndicator({ progress, scrollHint }: { progress: MotionValue<number>; scrollHint: string }) {
  ...
  return (
    <div
      ...
      className="absolute bottom-8 left-[21.5%] -translate-x-1/2 z-[15] flex flex-col items-center gap-2 pointer-events-none"
    >
      ...
      <span className="text-gray-600 text-xs font-semibold tracking-[0.15em] uppercase drop-shadow">{scrollHint}</span>
    </div>
  )
}

// ── DesktopHero ───────────────────────────────────────────────────────────────
// Wheel events are intercepted while animation is in progress.
// progress 0 → card on right side. progress 1 → full screen.
...
function DesktopHero({ onApply }: { onApply: () => void }) {
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      // At page top scrolling UP → re-lock and reverse animation
  ...
  // Left text: fade+slide out in first 50% of animation
  ...
  return (
    <div className="relative h-screen bg-white">
      ...
      {/* Video card — expands left from right side to full screen */}
      <motion.div
